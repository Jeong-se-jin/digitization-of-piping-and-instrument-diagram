...
from app.services.line_detection.detectors import detect_raw_segments, HOUGH
...
def detect_line_segments(pid_id: str,
    ...
    """
    logger.info(f'Starting to detect line segments using the {backend} backend')
    ...
    # to treat them as a single line.
    params = {
        'rho': rho, 'theta_param': theta_param, 'threshold': threshold,
        'min_line_length': min_line_length, 'max_line_gap': max_line_gap,
    }
    params.update(backend_params or {})
    hough_results_line_segments = detect_raw_segments(
        preprocessed_image, backend, **params)
