...
def pre_find_symbol_connectivities(
    ...
    include_all_symbols: bool = config.treat_all_symbols_as_assets,
    include_arrows: bool = config.treat_arrows_as_assets,
    ...
    for symbol_node_id, symbol_node in symbol_nodes:
        ...
        if symbol_label == arrow_symbol_label and not (
                include_all_symbols and include_arrows):
            continue

        has_text = (SymbolNodeKeysConfig.TEXT_ASSOCIATED_KEY in symbol_node and
                    symbol_node[SymbolNodeKeysConfig.TEXT_ASSOCIATED_KEY] is not None)

        if include_all_symbols:
            # Every detected symbol is an asset, tagged or not. An untagged
            # symbol still sits on the piping, and excluding it both loses a
            # node and breaks the run of line through it.
