...
def _display_text(asset_data):
    """A name for an asset, falling back for one with no usable tag.

    With treat_all_symbols_as_assets on, a node can reach the output without an
    associated tag; the response model needs a string, and "<class> #<id>" keeps
    it identifiable on the drawing.
    """
    text = asset_data.get(SymbolNodeKeysConfig.TEXT_ASSOCIATED_KEY)
    if text:
        return text
    label = asset_data.get(SymbolNodeKeysConfig.LABEL_KEY, 'symbol')
    return f"{label.rsplit('/', 1)[-1]} #{asset_data.get('id', '?')}"
    ...
    for asset_symbol_id, connected_nodes in symbol_connections.items():
        ...
        for traversal_connection in connected_nodes:
            ...
            connected_symbol = ConnectedSymbolsConnectionItem(
                ...
                text_associated=_display_text(asset_data),
                segments=segments,
        ...
        symbol_item = ConnectedSymbolsItem(
            ...
            text_associated=_display_text(container_asset_data),
